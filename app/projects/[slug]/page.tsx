...
export default async function ProjectPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="content-page project-page" id="main-content">
    <Breadcrumbs items={[{ label: "Home", href: "/" }, { label: "Projects", href: "/projects" }, { label: project.title }]} /><ProjectTracker slug={project.slug} />
    ...
    <section className="case-metrics" aria-label="Illustrative service metrics">{project.metrics.map((metric) => <article key={metric.label}><span>{metric.label}</span><strong>{metric.value}</strong><small>ILLUSTRATIVE</small></article>)}</section>
